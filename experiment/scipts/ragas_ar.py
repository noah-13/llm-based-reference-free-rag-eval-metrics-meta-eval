...
class AnswerRelevanceEvaluator:
    ...
    def _generate(self, answer: str, max_new_tokens=256):
        prompt = self.build_answer_relevance_prompt(answer)
        ...
        truncated = response.shape[0] >= max_new_tokens
        return self.tokenizer.decode(response, skip_special_tokens=True), truncated

    
    def build_answer_relevance_prompt(self, answer: str) -> str:
        instruction = "Given an answer, generate a question that the answer likely responds to, and classify whether the answer is noncommittal (e.g., vague, evasive, or ambiguous)."
    ...
    def evaluate(self, question: str, answer: str) -> dict:
        ...
        for _ in range(self.strictness):
            decoded, truncated = self._generate(answer)
        ...
        result["answer_relevance_score"] = float(sims.mean() * (1 - int(any(flags))))
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        try:
            ...
            for k in new_columns:
                if k == "answer_relevancy_score":
                    result[k] = np.nan
                elif isinstance(new_columns[k], list) and new_columns[k] and isinstance(new_columns[k][0], list):
                    result[k] = []
                else:
                    result[k] = 0
