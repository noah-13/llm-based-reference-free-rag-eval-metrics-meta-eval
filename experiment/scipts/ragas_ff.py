...
class FaithfulnessEvaluator:
    ...
    def evaluate(self, question: str, answer: str, context: str) -> dict:
        ...
        prompt = f"""Given a question and its answer, rewrite the answer into multiple self-contained factual statements without using any pronouns. Return only the list of statements in JSON format.
        ...
        decoded, truncated = self._generate(prompt, max_new_tokens=512)
        ...
        for stmt in statements:
            prompt = f"""For each given context and statement, determine whether the statement can be directly inferred from the context. Respond with 'Verdict: 1 or 0'.

            Context: {context}
            Statement: {stmt}
            """
            decoded_judge, truncated = self._generate(prompt, max_new_tokens=64)
            ...
            for line in decoded_judge.split("\n"):
                if re.search(r"verdict\s*[:：]\s*1\b", decoded_judge, re.IGNORECASE):
                    verdict = 1
