...
def extract_explicit_score_from_text(text: str, score_range=(1, 5)) -> float | None:
    valid_scores = [f"{i}" for i in range(score_range[0], score_range[1] + 1)]
    pattern = r"\b(" + "|".join(valid_scores) + r"(?:\.0|\.5)?)\b"
...
def build_prompt_variant(context, answer, mode="imperative", score_range=(1, 5)):
    score_str = f"{score_range[0]} (unfaithful) to {score_range[1]} (faithful)"
    instruction = {
        "imperative": f"Rate the faithfulness of the answer from {score_str}.",
        "interrogative": f"How faithful is the answer to the context on a scale from {score_str}?"
    }.get(mode)
    if instruction is None:
        raise ValueError("mode must be 'imperative' or 'interrogative'")
    return f"""{instruction}
    An answer is faithful if all its content can be fully inferred from the provided context. If an answer includes information not found in the context, it is less faithful.
...
def main(input_csv, output_csv, model_id):
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df)):
        c, a = row["context"], row["answer"]
        for mode, score_range in product(modes, ranges):
            prompt = build_prompt_variant(c, a, mode, score_range)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_csv", required=True, help="Path to input CSV with context, answer")
