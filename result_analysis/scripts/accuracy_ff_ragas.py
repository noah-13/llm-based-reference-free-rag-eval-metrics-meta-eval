...
for mode in ["", "chat_"]:
    df = pd.read_csv(f"../../experiment/output/ff_ragas_{mode}output.csv")

    final_df = pd.read_excel("final_annotated_with_final_answer.xlsx", sheet_name="Faithfulness")
    ...
    for i in range(50):
        ...
        score_a = row_a["faithfulness_score"]
        score_b = row_b["faithfulness_score"]
        ...
        try:
            output_parse_status_a = int(row_a["statements_parse_success"])
            output_parse_status_b = int(row_b["statements_parse_success"])
            output_parse_status_0 += output_parse_status_a+ output_parse_status_b
        except (ValueError, SyntaxError):
            pass
    output_parse_status_1 = 100 - output_parse_status_0
